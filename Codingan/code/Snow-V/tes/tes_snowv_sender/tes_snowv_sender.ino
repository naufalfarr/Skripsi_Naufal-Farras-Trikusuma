...
void sendEncryptedFragments(const uint8_t *ciphertext, size_t len) {
    ...
    while (offset < len) {
        ...
        offset += chunkSize;
        totalChunks++;
        delay(1); // Sangat penting untuk mencegah buffer overflow
    }
...
// Fungsi untuk mengenkripsi pesan dan mengembalikan ciphertext dan panjangnya
uint8_t* encryptMessage(const char* message, size_t &len) {
    len = strlen(message);
    uint8_t* ciphertext = new uint8_t[len];
    
    auto start = high_resolution_clock::now();
    snow_v.keyiv_setup(key, iv);
    snow_v.encrypt((const u8*)message, ciphertext, len);
...
void loop() {
    for (int i = 0; i < sizeof(plaintextSets) / sizeof(plaintextSets[2]); i++) {
        size_t len;
        ...
        delay(100); //Jeda setelah semua pesan dikirim
}