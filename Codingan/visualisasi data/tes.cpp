#include <ESP8266WiFi.h>
#include <espnow.h>
#include <Arduino.h>
#include <cstring>
#include <stdint.h>
#include <chrono>
using namespace std::chrono;

// Configuration
const int CLEFIA_BLOCK_SIZE = 16;
const int CLEFIA_KEY_SIZE = 32;
const int CLEFIA_ROUNDS = 32;

// Forward declarations
void clefiaDecrypt(uint32_t *plaintext, const uint32_t *ciphertext, const uint32_t *rk);
void clefiaKeySchedule(uint32_t *rk, const uint8_t *key);
void clefiaF(uint32_t *dst, const uint32_t *src, int offset);

// Transmission control
struct PacketHeader {
  uint16_t sequenceNumber;
  uint16_t totalPackets;
  uint16_t payloadSize;
};

// Global variables
uint32_t* roundKeys = nullptr;
uint8_t* decryptionBuffer = nullptr;
size_t currentDataSize = 0;
bool transmissionInProgress = false;

// CLEFIA Constants in PROGMEM
const PROGMEM uint32_t con256[60] = {
    0xf56b7aeb, 0x994a8a42, 0x96a4bd75, 0xfa854521,
    0x735b768a, 0x1f7abac4, 0xd5bc3b45, 0xb99d5d62,
    0x52d73592, 0x3ef636e5, 0xc57a1ac9, 0xa95b9b72,
    0x5ab42554, 0x369555ed, 0x1553ba9a, 0x7972b2a2,
    0xe6b85d4d, 0x8a995951, 0x4b550696, 0x2774b4fc,
    0xc9bb034b, 0xa59a5a7e, 0x88cc81a5, 0xe4ed2d3f,
    0x7c6f68e2, 0x104e8ecb, 0xd2263471, 0xbe07c765,
    0x511a3208, 0x3d3bfbe6, 0x1084b134, 0x7ca565a7,
    0x304bf0aa, 0x5c6aaa87, 0xf4347855, 0x9815d543,
    0x4213141a, 0x2e32f2f5, 0xcd180a0d, 0xa139f97a,
    0x5e852d36, 0x32a464e9, 0xc353169b, 0xaf72b274,
    0x8db88b4d, 0xe199593a, 0x7ed56d96, 0x12f434c9,
    0xd37b36cb, 0xbf5a9a64, 0x85ac9b65, 0xe98d4d32,
    0x7adf6582, 0x16fe3ecd, 0xd17e32c1, 0xbd5f9f66,
    0x50b63150, 0x3c9757e7, 0x1052b098, 0x7c73b3a7
};

// S-box untuk CLEFIA
const PROGMEM uint8_t S0[256] = {
      
  };

const PROGMEM uint8_t S1[256] = {
      
  }; 

// Helper functions
inline uint32_t rotl(uint32_t x, int s) {
    return (x << s) | (x >> (32 - s));
}

inline uint32_t rotr(uint32_t x, int s) {
    return (x >> s) | (x << (32 - s));
}

inline uint32_t pgm_read_con256(int index) {
    return pgm_read_dword(&con256[index]);
}

inline uint8_t pgm_read_S0(int index) {
    return pgm_read_byte(&S0[index]);
}

inline uint8_t pgm_read_S1(int index) {
    return pgm_read_byte(&S1[index]);
}

// Core CLEFIA Functions
void ICACHE_RAM_ATTR clefiaF(uint32_t *dst, const uint32_t *src, int offset) {
    uint32_t x = src[0] ^ pgm_read_con256(offset);
    uint32_t y = src[1];
    uint32_t z = pgm_read_con256(offset);

    y ^= rotr(x, 8) ^ rotl(x, 16) ^ rotl(x, 24);
    
    uint32_t temp = 0;
    temp |= ((uint32_t)pgm_read_S0((uint8_t)(y >> 24))) << 0;
    temp |= ((uint32_t)pgm_read_S1((uint8_t)(y >> 16))) << 8;
    temp |= ((uint32_t)pgm_read_S0((uint8_t)(y >> 8))) << 16;
    temp |= ((uint32_t)pgm_read_S1((uint8_t)y)) << 24;
    
    dst[0] = temp ^ z;
    dst[1] = y;
    
    yield();
}

void ICACHE_RAM_ATTR clefiaKeySchedule(uint32_t *rk, const uint8_t *key) {
    uint32_t L[4], R[4];
    
    for (int i = 0; i < 4; i++) {
        L[i] = ((uint32_t)key[4*i+3] << 24) | 
               ((uint32_t)key[4*i+2] << 16) |
               ((uint32_t)key[4*i+1] << 8) | 
               ((uint32_t)key[4*i]);
        yield();
    }

    for (int i = 0; i < CLEFIA_ROUNDS / 4; i++) {
        memcpy(R, L, 16);
        
        clefiaF(&R[0], &R[2], i*4);
        clefiaF(&R[2], &R[0], i*4+2);
        
        for (int j = 0; j < 4; j++) {
            rk[i*4 + j] = R[j];
        }
        
        if (i % 2 == 0) {
            for (int j = 0; j < 4; j++) {
                L[j] ^= rotl(R[j], 15);
            }
        } else {
            for (int j = 0; j < 4; j++) {
                L[j] ^= rotl(R[j], 17);
            }
        }
        
        yield();
    }

    for (int i = 0; i < 4; i++) {
        rk[CLEFIA_ROUNDS + i] = L[i];
    }
}

void clefiaDecrypt(uint32_t *plaintext, const uint32_t *ciphertext, const uint32_t *rk) {
    // Inisialisasi nilai plaintext dari ciphertext input
    uint32_t L[2], R[2];
    L[0] = ciphertext[0];
    L[1] = ciphertext[1];
    R[0] = ciphertext[2];
    R[1] = ciphertext[3];

    // Loop utama untuk ronde CLEFIA dalam urutan terbalik
    for (int r = CLEFIA_ROUNDS - 1; r >= 0; r--) {
        uint32_t tempL[2];
        
        // Fungsionalitas Feistel jaringan CLEFIA (f function)
        clefiaF(tempL, R, r);
        
        // XOR dengan round key rk dan gabungkan dengan blok L
        L[0] ^= tempL[0] ^ rk[r * 2];
        L[1] ^= tempL[1] ^ rk[r * 2 + 1];
        
        // Swap L dan R
        if (r != 0) {
            uint32_t tempSwap[2] = { L[0], L[1] };
            L[0] = R[0];
            L[1] = R[1];
            R[0] = tempSwap[0];
            R[1] = tempSwap[1];
        }
    }

    // Menyimpan hasil dekripsi pada output plaintext
    plaintext[0] = L[0];
    plaintext[1] = L[1];
    plaintext[2] = R[0];
    plaintext[3] = R[1];
}

// Callback untuk menerima data melalui ESP-NOW
void onDataRecv(uint8_t *mac, uint8_t *incomingData, uint8_t len) {
  static PacketHeader *header = nullptr;
    if (!transmissionInProgress) {
        PacketHeader *header = (PacketHeader *)incomingData;
        transmissionInProgress = true;
        currentDataSize = 0;
        decryptionBuffer = (uint8_t *)malloc(header->payloadSize);
    }

    if (decryptionBuffer != nullptr) {
        memcpy(decryptionBuffer + currentDataSize, incomingData + sizeof(PacketHeader), len - sizeof(PacketHeader));
        currentDataSize += len - sizeof(PacketHeader);

        if (currentDataSize >= header->payloadSize) {
            uint32_t plaintext[4];
            uint32_t *ciphertext = (uint32_t *)decryptionBuffer;
            
            auto encryptionStart = std::chrono::high_resolution_clock::now();
            
            Serial.print(F("Decrypted Data: "));
            
            // Melakukan dekripsi blok per blok
            for (size_t i = 0; i < currentDataSize; i += CLEFIA_BLOCK_SIZE) {
                clefiaDecrypt((uint32_t *)plaintext, ciphertext + (i / 4), roundKeys);
                
                // Cetak setiap blok yang didekripsi sebagai teks
                for (size_t j = 0; j < CLEFIA_BLOCK_SIZE; j++) {
                    Serial.print((char)plaintext[j]);
                }
            }
            Serial.println();  // Ganti baris setelah semua blok selesai
            
            auto encryptionEnd = std::chrono::high_resolution_clock::now();
            auto encryptionDuration = std::chrono::duration_cast<std::chrono::microseconds>(encryptionEnd - encryptionStart).count();
              Serial.print(F("Encrypted data size (bytes): "));
              Serial.println(currentDataSize);
              Serial.print(F("Encryption time (microseconds): "));
              Serial.println(encryptionDuration);              
            
            free(decryptionBuffer);
            transmissionInProgress = false;
        }
    }
}

void setup() {
    Serial.begin(115200);
    WiFi.mode(WIFI_STA);
    WiFi.disconnect();

    if (esp_now_init() != 0) {
        Serial.println("ESP-NOW Init Failed");
        return;
    }

    esp_now_set_self_role(ESP_NOW_ROLE_SLAVE);
    esp_now_register_recv_cb(onDataRecv);

    // Mengatur kunci enkripsi (sama seperti di sender)
    uint8_t key[CLEFIA_KEY_SIZE] = { /* Isi dengan kunci CLEFIA */ };
    roundKeys = (uint32_t *)malloc(sizeof(uint32_t) * CLEFIA_ROUNDS);
    clefiaKeySchedule(roundKeys, key);
}

void loop() {
    delay(100);
}
